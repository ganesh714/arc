...
public class AIController {
    ...
    @PostMapping("/agent")
    public ResponseEntity<?> agent(@RequestBody AIAgentRequest request) {
        try {
            if (request.getPrompt() == null || request.getPrompt().trim().isEmpty()) {
                return ResponseEntity.badRequest().body("Prompt cannot be empty");
            }
            ...
            if (request.getToolDefinitions() == null) {
                return ResponseEntity.badRequest().body("Tool definitions cannot be null");
            }
            
            String jsonTree = aiService.agentProcess(request.getPrompt(), request.getContextNodes(), request.getToolDefinitions());
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Failed to execute AI agent turn: " + e.getMessage());
        }
    }
}
