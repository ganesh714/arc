...
| High 7 | ~~**No Resilience in JWT Blacklist Check**~~ | ~~`JwtService.isTokenBlacklisted()` calls Redis `hasKey()` with no try/catch fallback. Redis downtime will break auth validation entirely.~~ | ✅ Resolved — Wrapped Redis checks in try-catch to allow DB fallback during Redis downtime |
...
| 2026-06-18 | High 7 | Wrapped Redis calls in `JwtService` with try-catch blocks to safely fallback to the PostgreSQL database if Redis goes offline. |
