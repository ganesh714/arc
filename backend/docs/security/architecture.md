...
4. Loom's `JwtAuthenticationFilter` reads the cookie, verifies the cryptographic signature, and authenticates the user.
...
Loom performs a cross-schema JPA query to verify the `jti` is not in this blacklist on every request, ensuring that intercepted tokens are instantly invalidated across services.
