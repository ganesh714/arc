...
| Med 3 | ~~**Over-aggressive `AuthContext.logout()`**~~ | ~~Calls `localStorage.clear()`, which wipes all browser storage for the origin, rather than just Arqulat Arc-specific state.~~ | ✅ Resolved — Removed `localStorage.clear()` to prevent wiping unrelated origin data |
